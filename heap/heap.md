...
> Status: partial。数组最小堆的向上调整、向下调整和建堆已实现；优先队列、堆排序说明和左式堆为学习笔记，其中堆排序实现在 `sort/`。
...
* 存储结构：二叉堆虽然是一颗完全二叉树，但它的存储方式一般不是链式存储，而是顺序存储。二叉堆中的所有结点都存储在数组中。假设一结点下标为i，则它的左孩子下标为2*i+1，右孩子下标为2*i+2；仅当i>0时，父节点下标为(i-1)/2，i=0的根结点没有父结点。
...
当前 `heap/heap.cpp` 实现的是数组最小堆：`upAdjust` 和 `downAdjust` 的时间复杂度为O(logn)、辅助空间为O(1)，其中 `downAdjust` 要求目标结点的两个子树已经满足最小堆性质；自底向上的 `buildHeap` 时间复杂度为O(n)、辅助空间为O(1)。空指针、非正长度和越界下标按 `heap.h` 的约定直接返回。
...
* 右侧链：从节点x出发，一直沿右分支前进所确立的那个分支。根节点的右侧链终点，必为全堆中最浅的外部节点。并且在包含n个节点的左式堆中右侧链长度为O(logn)。
