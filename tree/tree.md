...
> Status: partial。当前实现包含二叉树构造、深度、叶子计数、递归/迭代遍历、反转，以及AVL基本旋转；B-树、B+树、Huffman树、红黑树和伸展树为学习笔记。

以下名称仅保留为 notes-only 设计清单，不在 `binaryTree.h` 中声明，也没有实现：`initBiTree`、`destroyBiTree`、`clearBiTree`、`biTreeEmpty`、`root`、`value`、`assign`、`parent`、`leftChild`、`rightChild`、`leftSibling`、`rightSibling`、`insertChild`、`deleteChild`。

`createBiTree` 成功时返回一组由 `malloc` 分配的结点；当前仓库没有公开销毁函数，调用者必须自行按后序遍历逐结点调用 `free`。

对含n个结点、高度为h、最大层宽为w的当前链式实现：构造、深度、叶子计数以及每种遍历的时间复杂度均为O(n)；递归构造、深度、叶子计数、递归遍历和递归反转使用O(h)调用栈，迭代深度优先遍历/反转使用最坏O(h)显式栈，层序遍历使用O(w)队列。AVL的单次左旋或右旋为O(1)时间和O(1)空间；当前旋转函数只修改链接，不更新结点的 `bf`。
...
* 深度（depth）：树中结点的最大层次称为树的深度或高度。本文及当前 `biTreeDepth` 实现按层数计高度：空树为0，只有根结点的树为1。
...
1. 根结点若不是叶子，至少有两个孩子；根结点也可以是整棵树唯一的叶子，此时没有孩子。
2. 每个非根中间结点都包含k-1个关键码和k个孩子，其中⌈m/2⌉ <= k <= m。
3. 每个非根叶子结点包含k-1个关键码，其中⌈m/2⌉ <= k <= m；根为叶子时适用根结点例外。
4. 所有的叶子节点都位于同一层。
5. 每个节点中的元素从小到大排列，节点当中k-1个元素正好是k个孩子包含的元素的值域分划。

B-树主要用于弥合不同存储级别之间的访问速度差异，可通过较高扇出减少外部查找的I/O次数。含有N个关键码的m阶B-树，其高度由每个非根内部结点至少⌈m/2⌉个孩子这一约束控制；m固定时为O(log N)，更具体可写为O(log_{⌈m/2⌉} N)。
...
B+树是基于B-树的一种变体。内部结点只保存索引通常能提高扇出，叶子顺序链接也有利于范围查询；具体性能仍取决于数据规模、页大小和工作负载。一个m阶的B+树具有如下几个特征：
...
* 遍历二叉树（traversing binary tree）：如何按某条搜索路径巡访树中每个结点，使每个结点均被访问一次，而且仅被访问一次。实质上是对非线性结构进行线性化；除序列首元素没有直接前驱、末元素没有直接后继外，其余结点各有一个直接前驱和直接后继。遍历分为深度优先遍历（depth first search）和广度优先遍历（breadth first search）。
...
当每个结点具有唯一标识时，只需[先序|后序]遍历序列和中序遍历序列即可唯一还原二叉树的拓扑结构；存在不可区分的重复值时不能保证唯一。
...
单调性：按上述严格不等式定义，BST不接受重复关键码，其中序遍历序列必然严格递增；反之，中序序列严格递增的二叉树满足该BST顺序性。对n个互异关键码，可以生成catalan(n)=(2n)!/(n!*(n+1)!)棵拓扑结构互异的BST。若要允许重复关键码，必须另外规定重复值固定进入左侧或右侧，并相应修改不等式。

等价BST：结构不尽相同的BST，中序遍历序列完全相同。特点：上下可变（联接关系不尽相同，承袭关系可以颠倒），左右不乱（中序遍历序列完全一致，全局严格递增）。任何一对等价BST之间的相互转换都可以视作是由一系列的基本操作串接而成的。而这种基本的变换无非两类，彼此对称。左旋和右旋。

退化问题：假设在一个BST中所有结点的度都不超过1，也就是说这棵树实际上已经退化成一条单链。按层数计高度时h=n，线性正比于n。结点数目固定时，兄弟子树高度越接近（平衡），全树也倾向于更低。由n个结点组成的二叉树，最小可能高度为⌈log2(n+1)⌉；达到该下界时可称为理想平衡。

#### 平衡二叉排序树（balance binary sort tree）
平衡二叉树排序树是一种结构平衡的二叉排序树，它的左子树和右子树都是平衡二叉树；且左子树和右子树的深度之差的绝对值不超过1。

##### AVL树
AVL树是最早被发明的自平衡二叉查找树。按本文“空树高度为0、单结点高度为1”的约定，高度为h的AVL树至少包含S(h) = fib(h+2) - 1个结点（fib(1)=fib(2)=1）。
* 平衡因子（balance factor）：该结点的左子树深度减去它的右子树深度。因此平衡二叉树上的所有结点的平衡因子只可能是-1，0，1。只要树上有一个结点的平衡因子绝对值大于1，则该二叉树就不是平衡的。
* 旋转（rotate）：当插入或删除使某结点的平衡因子绝对值达到2时，需要重新平衡以该结点为根的子树。插入时，修复从插入位置向上遇到的最低失衡结点后，子树高度恢复，通常无需继续向上传播；删除时，旋转后子树高度仍可能降低，因此必须继续向祖先检查，可能发生多次再平衡。假设插入后最低失衡子树的根为a，局部调整分为4种情况：
...
在真正实现算法时，可以对以上旋转操作进行“3+4”重构。即对3个节点和4棵子树的中序遍历重新拼接起来一颗子树。
...
红黑树插入和删除的搜索及修复总时间为O(logN)；旋转次数有常数上界，但重新着色可能沿祖先路径传播。红黑树可以理解为提升各红节点，使之与其（黑）父亲等高——于是每棵红黑树都可对应到一棵2-3-4树。红黑树的高度h = O(logN)。

双红（double red）：相邻的父子节点都为红色。只需常数时间“3+4”重构（旋转）和染色的局部操作。

双黑（double black）：删除黑结点后，为描述某条路径暂时少一个黑结点而使用的修复状态。单次局部旋转和染色为O(1)，但双黑可能向根传播，所以完整删除修复最坏为O(logN)。
