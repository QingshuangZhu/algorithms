...
> Status: partial。`sort.h` 中的内部排序接口均有实现；外部排序部分仅为学习笔记。
...
### 归并排序
归并排序（merging sort）：是将两个或两个以上的有序表组合成一个新的有序表，归并算法的核心思想是分治法（Divide and Conquer）。假设初始数列含有n个记录，则可以看成是n个有序的子序列，每个子序列的长度为1，然后两两归并，得到⌈n/2⌉个长度为2或1的有序子序列；再两两归并，如此重复直至得到一个长度为n的有序序列为止，这种排序方法称为2-路归并排序。n>0时，完整归并需要进行⌈log2n⌉趟。

### 计数排序
计数排序（counting sort）：是一个非基于比较的排序算法。在对范围大小为k的整数排序时，其时间复杂度为O(n+k)；只有当k相对n足够小时，它才具有相对于一般比较排序的复杂度优势。

### 桶排序
桶排序（bucket sort）：可以看成是计数排序的升级版，遍历数据把其分到多个有限个对应的桶里，然后对每个非空的桶进行排序，并把每个非空桶的数据依次取出，即可完成排序。

当前 `bucketSort` 建立n个桶，并用链表归并排序处理每个桶。其一般时间界为O(n+k+Σmᵢlogmᵢ)，其中k为桶数、mᵢ为第i个桶的元素数；当前k=n，最坏时间为O(nlogn)。元素以头插法进入桶，因此相等元素的原始相对次序会被反转，当前实现不稳定。
...
* 最低位优先（least significant digit first）：先对最低有效位进行稳定排序，然后按从低到高的顺序依次对更高有效位重复稳定排序，最后得到有序序列。

当前 `radixSort` 是十进制 LSD 实现，只接受非负整数；若输入包含负数，函数不执行排序。

当前接口约定详见 `sort.h`：长度为0时允许空指针；区间端点均为闭区间。`mergeSort`/`mergeSort2` 要求调用者提供覆盖同一索引范围且不与原数组重叠的临时缓冲区；`countSort` 将结果写入独立输出数组，当值域差不受支持或分配失败时保持输出不变。`bucketSort` 和 `radixSort` 在分配失败时保持输入不变。
...
其中d为最大关键字的位数，b为基数。上表冒泡排序的最好O(n)适用于带提前终止的 `bubbleSort2` 和 `cocktail`；不带提前终止的 `bubbleSort` 在已排序输入上仍执行Θ(n²)次比较。`binaryInsertionSort` 使用二分查找确定稳定插入点，比较次数为Θ(nlogn)，但平均和最坏移动次数仍为Θ(n²)。

按辅助空间为O(1)的严格定义，直接/二分插入、希尔、三种冒泡/鸡尾酒、简单选择和堆排序原地工作。三种快速排序都在原数组中完成分区，但递归调用栈或显式栈平均为O(logn)、最坏为O(n)，因此若计入控制栈便不满足严格的O(1)原地定义。归并、计数、桶和基数排序使用随输入增长的元素缓冲区，不属于原地排序；递归归并排序还会使用O(logn)调用栈。
